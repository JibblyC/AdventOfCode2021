...
for index in range(len(p1Array)):
    ...
    #Check Diagonal
        ...
        differenceX = p1Array[index][0] - p2Array[index][0]
        differenceY = p1Array[index][1] - p2Array[index][1]
        for subIndex in range(abs(differenceX) + 1):
            if differenceX > 0 and differenceY > 0:
                pointToUpdate = [p1Array[index][0] - subIndex,p1Array[index][1] - subIndex]
